...
const exportToExcelStyled = async (dataRows, districtName) => {
  ...
  saveAs(blob, `Account_Mapping_${districtName}_${new Date().toISOString().slice(0, 10)}.xlsx`);
};

const AccountMappingReport = () => {
    ...
    <Box sx={{ p: { xs: 2, md: 3 } }}>
      ...
      <Stack direction="row" justifyContent="space-between" alignItems="flex-start" sx={{ mb: 3, flexWrap: "wrap", gap: 2 }}>
        <Box>
          ...
          <Breadcrumbs sx={{ mt: 0.5 }}>
            ...
            <Typography color="text.primary">Account Mapping</Typography>
          </Breadcrumbs>
        </Box>
      ...
      {!loading && !error && (
        <Stack direction="row" spacing={1.5} sx={{ mb: 2, flexWrap: "wrap", gap: 1 }}>
          ...
          {district && <Chip label={`District: ${district}`} color="info" size="small" onDelete={() => { setDistrict(""); setPage(0); }} />}
...
export default AccountMappingReport;
